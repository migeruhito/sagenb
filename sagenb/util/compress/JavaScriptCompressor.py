...
class JavaScriptCompressor:
    ...
    __totalSources = 0
    __sources = []
    __delimeter = [{"name": "doublequote", "start": '"', "end": '"', "noslash": True}, {"name": "singlequote", "start": "'", "end": "'", "noslash": True}, {"name": "singlelinecomment",
    ...
    def __clean(self, str):
        ...
        for a in range(0, len(map)):
            type = map[a]["name"]
            if type == "code":
                clean.append(self.__addCleanCode(
                    str[map[a]["start"]:map[a]["end"]]))
    ...
    def __sourceManager(self, jsSource):
        ...
        if type(jsSource) == striType:
            ...
            self.__sourcePusher(jsSource.get("code", ""),
                                jsSource.get("name", ""))
            ...
            for a in range(0, b):
                if type(jsSource[a]) == dictType and jsSource.get("code", None) != None and jsSource.get("name", None) != None:
                    self.__sourcePusher(jsSource.get(
                        "code", None), jsSource.get("name", None))
