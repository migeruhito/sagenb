...
class ResultsParser(object):
    ...
    def __init__(self, images_dir):
        # Order matters, place more restrictive regex's before more general
        # ones If no regex matches, line will be discarded a self transition is
        # needes to produce any output
        self.transitions = {
            States.NORMAL: [
                # IMAGE
    ...
    def parse(self, text):
        ...
        for line in text.splitlines():
            for (regex,
                 replacement,
                 line_type,
                 new_state) in self.transitions[state]:
                if regex.match(line):
                    result = result_plain if line_type in (
                            LineTypes.PLAIN, LineTypes.HTML)\
                        else result_show
...
def results2rst(text, images_dir):
    ...
    EXAMPLES::
        ...
        sage: s="<html><font color='black'><img src='cell://sage0.png'>"\
                "</font></html>"
        sage: results2rst(s,'')
        '\n.. image:: sage0.png\n    :align: center\n'
        sage: results2rst("4",'')
