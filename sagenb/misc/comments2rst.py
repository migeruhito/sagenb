...
def preprocess_display_latex(text):
    ...
    EXAMPLES::
        ...
        sage: from sagenb.misc.comments2rst import (
            preprocess_display_latex) # optional - beautifulsoup
...
def replace_courier(soup):
    ...
    for t in soup.findAll(
            lambda s: 'style' in s and 'courier' in s['style']):
        tag = Tag(soup, 'code')
        while t.contents:
...
def replace_latex(soup):
    ...
    EXAMPLES::
        ...
        sage: from sagenb.misc.comments2rst import (
            replace_latex) # optional - beautifulsoup
        sage: from BeautifulSoup import (
            ICantBelieveItsBeautifulSoup) # optional - beautifulsoup
        sage: s = ICantBelieveItsBeautifulSoup(
            "<p>Some <strong>latex: $e^\pi "
            "i=-1$</strong></p>") # optional - beautifulsoup
        sage: replace_latex(s) # optional - beautifulsoup
        sage: s # optional - beautifulsoup
        <p>Some <strong>latex: :math:`e^\pi i=-1`</strong></p>
        sage: s = ICantBelieveItsBeautifulSoup(
            "<p><strong>2+2 | 1+3</strong></p>") # optional - beautifulsoup
        sage: replace_latex(s) # optional - beautifulsoup
        sage: s # optional - beautifulsoup
...
class Soup2Rst(object):
    ...
    def visit(self, node):
        if isinstance(
                node, (CData, Comment, Declaration, ProcessingInstruction)):
            return ''
            ...
            try:
                ...
                print('Warning: node not supported (or something else?) ' +
                      node.name)
                return unicode(node)
        else:
            # Assume plain string
    ...
    def visit_img(self, node):
        return '.. image:: ' + os.path.join(
            self.images_dir, node['src'].replace(
                ' ', '_')) + '\n    :align: center\n'
    ...
    def visit_em(self, node):
        if node.contents:
            return ' *' + ' '.join(
                    self.visit(tag) for tag in node.contents).strip() + '* '
        else:
            return ''
...
def html2rst(text, images_dir):
    ...
    EXAMPLES::
        ...
        sage: from sagenb.misc.comments2rst import (
            html2rst) # optional - beautifulsoup
        sage: html2rst(
            '<p>Some text with <em>math</em>: $e^{\pi i}=-1$</p>',
            '') # optional - beautifulsoup
        u'Some text with  *math* :  :math:`e^{\\pi i}=-1`\n\n'
        sage: html2rst(
            '<p>Text with <em>incorrect</p> nesting</em>.',
            '') # optional - beautifulsoup
        ...
        u'\xe1\xf1 \xf1\xe1'
        sage: html2rst(
        '<p>some text</p><p>$$</p><p>3.183098861 \cdot 10^{-1}</p><p>$$</p>',
        '') # optional - beautifulsoup
