...
def get_theme_template(app, theme, template_name, _fallback=True):
    """
    As flask.templating.Environment.get_template(), but returns if possible a
    theme'd template.  If ``_fallback`` is True and the template does not exist
    within the theme, it will fall back on trying to render the template using
    the application's normal templates.
    ...
    - ``theme`` - Either the identifier of the theme to use, or an actual
      `Theme` instance.
