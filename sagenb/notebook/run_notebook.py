...
class NotebookRun(object):
    config_stub = '\n'.join((
        ...
        'sagenb.notebook.misc.DIR = %(cwd)r '
    ...
    def prepare_kwds(self, kw):
        ...
        if kw['automatic_login']:
            ...
            if kw['upload']:
                ...
                kw['open_page'] += "; open_page('%(hostname)s', %(port)s,"\
                                   " %(secure)s, %(start_path)s)" % kw
...
class NotebookRunTornado(NotebookRun):
    ...
    def run_command(self, kw):
        ...
        with open(run_file, 'w') as script:
            script.write(
                (self.config_stub + self.TORNADO_NOTEBOOK_CONFIG) % kw)
...
class NotebookRunFlask(NotebookRun):
    ...
    FLASK_NOTEBOOK_CONFIG = '\n'.join((
        ...
        '',
        '    # Monkey patching idiom: http://mail.python.org/pipermail/'
        'python-dev/2008-January/076194.html',
...
class NotebookRunTwisted(NotebookRun):
    ...
    TWISTD_NOTEBOOK_CONFIG = '\n'.join((
        ...
        '# be the default reactor when needed.',
        '#See http://twistedmatrix.com/trac/ticket/5478',
        'import sys, platform',
    ...
    def run_command(self, kw):
        ...
        if platformType != 'win32':
            ...
            try:
                ...
                if str(e).startswith('Another twistd server is running,'):
                    ...
                    old_interface, old_port, old_secure = (
                        self.get_old_settings(conf))
                    ...
                    print('\n'.join((
                        '',
                        'Please either stop the old server or run the new',
        ...
        if kw['secure']:
            kw['strport'] = 'ssl:%(port)s:interface=%(interface)s:privateKey='\
                            '%(private_pem)s:certKey=%(public_pem)s' % kw
        else:
        ...
        if kw['profile']:
            profilecmd = '--profile=%s --profiler=cprofile'\
                         ' --savestats' % self.profile_file(kw['profile'])
        else:
            profilecmd = ''
        cmd = 'twistd %s --pidfile="%s" -ny "%s"' % (profilecmd,
                                                     kw['pidfile'], conf)
        return cmd
...
command = {'flask': NotebookRunFlask,
           'twistd': NotebookRunTwisted,
           'uwsgi': NotebookRunuWSGI,
...
def notebook_run(self,
    ...
    if subnets is not None:
        raise ValueError(
            ...
            'firewall to block subnets, or even better, volunteer to write '
            'the code to implement subnets again.')
    if require_login is not None or open_viewer is not None:
        raise ValueError(
            'The require_login and open_viewer parameters are no longer '
            'supported. Please use automatic_login=True to automatically log '
            'in as admin, or use automatic_login=False to not automatically '
            'log in.')
    ...
    if reset:
        ...
        if nb.user_manager().user_exists('admin'):
            ...
            if secure:
                print('Login to the Sage notebook as admin with the password'
                      ' you specified above.')
        #nb.del_user('root')
...
def get_admin_passwd():
    ...
    while True:
        ...
        if len(passwd) < min_password_length:
            print('That password is way too short. Enter a password with at '
                  'least %d characters.' % min_password_length)
            continue
